...
const Navbar = () => {
  ...
  const [notification, setNotification] = useState<Array<Record<string ,string | boolean>> | undefined>([]);
  ...
  return (
    <div className=" sticky left-0 top-0 p-5 flex pt-4 pb-4 items-center border-b-2 z-100 bg-white-1 border-white-3 justify-between  w-full">
      ...
      <div className="flex  gap-4 items-center">
        <div className="relative">
          <div onClick={toggleNotification} className=" relative cursor-pointer">
            { unRead != 0 &&
              <span className="absolute min-w-3 h-3  text-sm text-white-1 rounded-full p-1 py-2 -right-2 top-0 bg-red-500 flex justify-center items-center font-bold">{unRead}</span>
            }
            <Icon.bell clicked={clicked} />
          </div>
          <NotificationComp clicked={clicked}  setClicked={setClicked} notification={notification} />
        </div>
          <div
          ...
          >
              ...
              <Popup open={optWrite}>
                  <ul className="flex flex-col gap-3 text-black-1 ">
                      <li onClick={() => navigate(`/new`)} className="border-b-2
                    hover:text-white-2 cursor-pointer w-max border-orange-1 pb-3">
                       <span >
                          Write Story
                       </span>
                      </li>
                      <li onClick={handleClick} className="cursor-pointer w-max hover:text-white-2">
                          <span>Generate Story</span></li>
                  </ul>
              </Popup>
          </div>

          <div onClick={() => setOptOpen(!optOpen)}
              ...
              <Popup open={optOpen}>
                  <ul className="flex flex-col gap-3 text-black-1 ">
                      <li onClick={() => navigate(`/Profile/${currentUser ? currentUser.uid : ""}/about`)} className="border-b-2
                    hover:text-white-2 cursor-pointer border-white-3 pb-3">
                       <span>
                          My Profile
                       </span>
                      </li>
                      <li onClick={handleClick} className="cursor-pointer hover:text-white-2"><span>Log_Out</span></li>
                </ul>
            </Popup>
         </div>
      </div>
