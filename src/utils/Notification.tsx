...
export async  function  sendNewNotfication(currentUser:UserType,PartId : string){
    try{
        ...
        if(followers){
            followers.forEach((follower) => {
               const NotId  = db.collection("user").doc(follower).collection("Notification").doc().id;
               db.collection("user")
                 .doc(follower)
...
export async  function MarkAsRead(currentUser : UserType ,  notificationId:string){
 
  if(!notificationId) return
  try{
    await db.collection("user").doc(currentUser?.uid).collection("Notification").doc(notificationId).update({
      read:true
    })
  }
  catch(error){
